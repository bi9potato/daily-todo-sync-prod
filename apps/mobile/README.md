...
Android 构建成功并上传 Release 后，工作流会把公开版本清单同步到 API：

```text
GET /api/mobile/releases/latest
```

App 会在“我的 → 应用更新”自动检查构建 SHA。发现新版本后，点击“下载并安装”会打开
固定 APK 地址，下载完成后由 Android 系统确认覆盖安装。普通 App 无法静默安装更新；
仓库为私有时，浏览器可能要求先登录 GitHub。

CI 会复用同一份内部测试签名。首次切换到该签名的 APK 可能需要先卸载旧测试版；之后可
直接覆盖安装。正式对外发布时应改用独立 release keystore，并安全备份签名材料。
